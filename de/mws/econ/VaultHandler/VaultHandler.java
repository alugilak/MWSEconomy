package de.mws.econ.VaultHandler;
...
{
    private final String name = "MWSKonto";

    private konto plugin;

    public VaultHandler( konto plugin )
    {
        this.plugin = plugin;

        Bukkit.getServer().getPluginManager().registerEvents( new EconomyServerListener(), plugin );

        plugin.log( "Vault support aktiviert." );
    }
    ...
    private EconomyResponse withdraw( String playerName, String uuid, double amount )
    {
        ...
        if( account.has( amount ) )
        {
            ...
            return new EconomyResponse( amount, account.getkonto(), ResponseType.SUCCESS, "" );
        }
        else
        {
            return new EconomyResponse( 0, account.getkonto(), ResponseType.FAILURE, "Insufficient funds" );
        }
    }
    ...
    {
        return new EconomyResponse( 0, 0, ResponseType.NOT_IMPLEMENTED, "MWSKonto does not support bank accounts!" );
    }
    ...
    public class EconomyServerListener implements Listener
    {
        ...
        public void onPluginEnable( PluginEnableEvent event )
        {
            if( plugin == null )
            {
                Plugin plugin = Bukkit.getServer().getPluginManager().getPlugin( "MWSKonto" );

                if( plugin != null && plugin.isEnabled() )
                {
                    VaultHandler.this.plugin = ( konto ) plugin;

                    VaultHandler.this.plugin.log( "Vault support aktiviert." );
                }
            }
        }
        ...
        public void onPluginDisable( PluginDisableEvent event )
        {
            if( plugin != null )
            {
                if( event.getPlugin().getDescription().getName().equals( name ) )
                {
                    ...
                    Bukkit.getLogger().info( "[MWSKonto] Vault support deaktiviert." );
                }
            }
        }
    }
